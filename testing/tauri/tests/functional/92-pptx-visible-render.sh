#!/bin/bash
# Test that PPTX documents are visibly rendered (not 0x0 dimensions)

source "$(dirname "$0")/../lib/test-client.sh"

TEST_LABEL="pptx-visible-render"
TMP_DIR=""
FILENAME="visible-test.pptx"

setup() {
  TMP_DIR=$(mktemp -d /tmp/ob-pptx-visible-XXXXXX)
  test_invoke "create_document" "{\"path\": \"$TMP_DIR/$FILENAME\"}" > /dev/null 2>&1
  OPEN_RESP=$(open_context_window "$TMP_DIR" "$TEST_LABEL" "PPTX Visible Render")
...
tab_label=$(test_js "Array.from(document.querySelectorAll('.tab-label')).find(t => t.textContent.includes('pptx') || t.textContent.includes('PowerPoint'))?.textContent || 'not found'" "$TEST_LABEL" | jq -r '.result')
if [ "$tab_label" = "not found" ]; then
  echo "TEST FAILED: No PowerPoint tab found" >&2
  exit 1
fi
echo "  [0;32m✓[0m PowerPoint tab present: $tab_label"
