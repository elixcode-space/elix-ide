...
setup() {
  ...
  open_context_window "$TMP_DIR" "$TEST_LABEL" "Docx No Bin" >/dev/null 2>&1
...
assert_no_binary_warning() {
  ...
  assert_not_empty "$txt" "Rendered DOCX should not be empty"
}
...
test_01_open_valid_docx_no_binary_warning() {
  open_docx_editor >/dev/null 2>&1
  sleep 1
