...
function createProvider() {
  const onDidChangeCustomDocument = new vscode.EventEmitter();
  return {
    openCustomDocument: async (uri, openContext, _token) => {
      const data = openContext.backupId ? await vscode.workspace.fs.readFile(vscode.Uri.parse(openContext.backupId)) : await vscode.workspace.fs.readFile(uri);
    ...
    resolveCustomEditor: async (document, webviewPanel, _token) => {
      ...
      const post = (type, payload = {}) => webview.postMessage({ type, ...payload });
      const update = () => {
...
async function activate(context) {
  const providers = [{ vt: "elixide.docxBinary" }, { vt: "elixide.xlsxBinary" }, { vt: "elixide.pptxBinary" }];
